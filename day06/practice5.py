...
# 이름(name)과 인사말(message) 두 개의 매개변수를 받는 함수를 작성하시오.
# 이때 message는 기본 매개변수로 설정하여 값이 입력되지 않으면 "안녕하세요"가 출력되도록 하시오.
...
def get_ticket_price(age):
    ...
    if age_int < 8:
        ...
        print('10,000원')
get_ticket_price(100)

# 문제 5: 가변 인자 합계 구하기 (sum_all)
# **전개 매개변수(*args)**를 사용하여 전달받는 숫자의 개수에 상관없이 모든 숫자의 합계를 반환하는 함수를 작성하시오.
def sum_all(*args):
    count = 0
    for i in args:
        count += i
    print( count )
sum_all(100,200,300,400)
...
def find_longest(*word):
    ...
    for i in word:
        if len_sum < len(i):
            ...
            len_word = i
    print('결과:',len_word)
find_longest('q21','tyui','opasdf','asfafdfsafds')
...
a_7=[{'name': 'A', 'score': 80}, {'name': 'B', 'score': 95}]
get_top_student(a_7)
...
# 문제 9: 전역 변수 점수 관리
# 전역 변수 current_score = 0을 선언하고, 점수를 추가하는 add_score(n) 함수와 감점하는 sub_score(n) 함수를 작성하시오.
# (함수 내에서 global 키워드를 사용하여 전역 변수 값을 수정하시오.)
current_score = 0
def add_score(n):
...
def calculate_payment( price , count , discount=0.1):
    money = price*count*(1-discount)
    print(int(money))

calculate_payment(10000,4)